...
## 2026-09-14 – Öffentliche Anime-Detailseite: Zustands- und Auslieferungsgrenzen (158/159)

`/anime/[id]` bleibt die kanonische Anime-Route mit strikt positiver, vollständig validierter ID. Autoritative Projekt-Slugs bilden die bestehenden Pretty-Projektlinks; numerische Compatibility-Routen bleiben verfügbar. Bewertungen und Viewzahlen ohne autoritative Quelle werden nicht als echte Kennzahlen dargestellt. Neue Rating-, View-, Emby- oder Anime-Slugregeln gehören nicht zu diesem Auftrag.

Die bestehende Episoden-API erhält eine ausdrücklich gewählte öffentliche Cursorprojektion: standardmäßig 24, höchstens 100 atomare Varianten- oder neutrale Episodenzeilen. Counts werden vor Cursor/LIMIT berechnet; Gruppen werden nach der begrenzten Seite aggregiert. Die volle Adminprojektion und der neutrale AnimeDetail-Fallback bleiben erhalten. Diese Rowgrenze ist keine globale Bytegarantie und begrenzt nicht die komplette neutrale AnimeDetail-Liste. `variant_id` und `release_version_id` sind getrennte Identitäten; die Anime-Seite übergibt beide an die vorhandene Streamkette. Der alte Aufruf ohne Selector bleibt eine bewusst nicht umgestaltete, weiterhin mehrdeutige Compatibility-Grenze. Segmentdaten kommen bei fachlichem Bedarf aus `theme_segment_assignments`, nicht aus Range-/Versionslabel-Heuristiken. Consumer-Matrix: `.planning/phases/159-public-anime-detail-konsolidierung/159-01-CONSUMERS.md`.

Der bestehende FansubVersionBrowser besitzt Gruppenauswahl und Public-Paging. Story, Filter und Varianten folgen demselben SSR-deterministischen Zustand. Nur ausdrückliche Auswahl wird geschützt gespeichert; Multitab-Ereignisse übernehmen nur aktuelle gültige IDs. Kein Storagepoll und keine fachlichen Requests beim Gruppenwechsel. Gridnachbarn werden erst bei Interaktion geladen und führen Zielanime und zugehörige Gridseite gemeinsam weiter.

Der bestehende Manifestcache bleibt geteilt und erhält 60 Sekunden Freshness, höchstens 20 ungenutzte erfüllte Einträge sowie referenzgezielten Abort/Retry. Die Anime-Coverausgabe ist auf 512 px bei Qualität 75 begrenzt. Provider verwenden echte Providerparameter. Lokale Bilder und API-Files nutzen eine explizite, frontendseitige Displayausgabe innerhalb bestehender Medienseams; `/covers/display/[file]` vermeidet den Vorrang statischer Dateien und Dateipfade als Verzeichnisse. API-Files bleiben über den vorhandenen Proxy an dessen feste interne Origin gebunden; keine Next-LocalIP-/Hostfreigabe wird erweitert.

Ein gemeinsamer Sharp-Helper begrenzt Eingabe, Dekodierung, Ausgabe, Zeit und parallele Arbeit für diese Quellen. Er liefert ein statisches erstes Bild, entsprechend der vorhandenen Upload-Thumbnail-Semantik; Originaldateien werden nicht verändert. Erfolgreiche API-File-Displayantworten bleiben privat/no-store. Ohne Display-Opt-in gelten bestehende Original-, Video-/Range- und Backendverträge. Der additive Displayvertrag ist ausdrücklich frontendseitig dokumentiert; der Backend-Dateiendpunkt wird dadurch nicht zum Transformationsendpunkt. Die defekte alte JPG-Platzhalterdatei wird nicht pauschal ersetzt; nur die Anime-Displayauflösung nutzt eine gültige statische PNG-Quelle. Details und Grenzen: `docs/api/api-contracts.md` und `159-04-DELIVERY.md`.

Genau zwei GSD-Phasen, 158 und 159, tragen den Auftrag. Technische Prüfergebnisse sind kein Human-UAT-Sign-off; 156 GAP02 und 157-06 Task4 sowie gesonderte neue menschliche Abnahmen bleiben separat offen. Keine Migration, Datenübernahme, Schema-/Release-/Medienlöschung oder produktive Datenmutation ist Bestandteil dieser Entscheidungen.


## 2026-09-14 – Eindeutige OP-/ED-Zuordnung je Release-Version (Phase156 GAP-04 / Quick260914-fc1)

Der Live-UAT-Auftrag begrenzt jeden OP- und ED-Platz auf ein Segment je echter Release-Version. Die vorhandene kanonische Backend-Typklassifikation wird wiederverwendet; Episodennummer, Segmentname und numerische Katalog-ID sind kein eigener Slot-Schlüssel. OP und ED sind unabhängig, weitere Segmenttypen werden nicht pauschal eingeschränkt.

Bereichsautomatik überspringt bereits durch ein anderes Segment derselben Familie belegte Zielversionen und meldet die tatsächlichen Ausnahmen. Einzel-/vollständig belegte Ziele führen zum Rollback der Segmentmutation. Eigene Zuordnungen bleiben idempotent; bestehende Konfliktdaten werden weder überschrieben noch pauschal bereinigt. Metadaten und Assignmentabgleich werden in einer Transaktion gespeichert, konkurrierende Schreiber am vorhandenen Anime-Datensatz serialisiert. Anzeigen verwenden Assignments; ein Bereich darf weiterhin einen Vorschlag begründen, aber keine tatsächliche Zuordnung vortäuschen.

Beim späteren Release-Import bleibt ein freier OP-/ED-Platz mit mehreren passenden Kandidaten unzugewiesen, bis ein Segment ausdrücklich gewählt wird. Es existiert keine belegte Vorrangregel; daher kein beliebiger Gewinner nach Name, ID oder Datum. Eine vorhandene Zuordnung bleibt erhalten, ein eindeutiger anderer Typ bleibt möglich. Andere Anime werden vor Varianten-Locks abgewiesen, statt einen Stream in die falsche Domain zu übernehmen. Origin und Contributors werden nicht automatisch umgehängt. Keine Migration oder Live-Datenbereinigung; technische Verifikation ersetzt keine offene Human-UAT-Abnahme.

Live-UAT-Nachtrag desselben Auftrags: Beim Wiederöffnen des Editors werden Von/Bis aus der ersten/letzten tatsächlichen zugewiesenen Folge vorbelegt, sofern sämtliche zugewiesenen Versionen verlässliche positive ganzzahlige Episodenlabels besitzen. Nur Folge 6 ergibt 6/6 statt des zuvor angefragten 2–6. Lücken bleiben tatsächliche Lücken; keine Herleitung aus Release-IDs. Fehlende/unvollständige oder nicht numerische Metadaten sowie reine Planungen behalten die gespeicherte Vorgabe. Öffnen ändert keine Daten; ausdrückliches Speichern übernimmt die sichtbaren Grenzen und kann damit einen vorher größeren Automatikbereich ersetzen.


## 2026-09-14 – Release-Datumsregeln und Mediengalerie (Quick260914-gif)

Auf bestätigten Nutzerauftrag gilt innerhalb einer echten Release-Version: Bearbeitungsabschluss darf nach UTC-Kalendertagen nicht vor Bearbeitungsbeginn liegen. Gleicher Tag ist zulässig, fehlende Daten bleiben unbekannt. Partielle Patches werden unter der bestehenden Transaktionssperre mit dem gespeicherten Gegenwert geprüft. Zwischen Folgen sind abweichende Arbeitsreihenfolgen ausschließlich Hinweise, keine Sperren, automatischen Korrekturen oder interpolierten Datumswerte. Beginn wird mit Beginn und Abschluss mit Abschluss verglichen.

Der vorhandene gemeinsame Editorkontext liefert maximal vier Vergleichsanker je persistierter Gruppe: spätester bekannter Tag früherer und frühester bekannter Tag späterer numerischer Folgen, jeweils pro Datumsfeld. Gleicher Anime, gleiche Gruppe und exakt gleiches getrimmtes Versionslabel sind erforderlich. Nicht numerische Specials erhalten keine erfundene Chronologie. Anker sind ein Kontext-Snapshot, ohne Polling oder neue Requests pro Datumseingabe. Die bestehenden Phase-143-Felder und deren Semantik bleiben maßgeblich: production_started_on („Bearbeitung begonnen am“) und release_date („Bearbeitung abgeschlossen am“); keine neue Veröffentlichung-/Abschlussmodellierung.

Die bestehende Releaseversion-Mediengalerie nutzt zwei Spalten mobil, ab44rem eigenem Container drei und ab59rem vier. Bilder bleiben4:3, Titel zeigen höchstens zwei und Beschreibungen drei Vorschauzeilen. Volltext bleibt im existierenden Detaildrawer; Metadaten umbrechen und Vorschauaktionen behalten kompakte Eigenhöhe am Kartenende. Uploads, Rollen, Vorschauauswahl und Medienownership ändern sich nicht. Keine Live-Daten-/Schemaänderung; offene Human-UAT156/157/158/159 bleiben offen.

## 2026-09-15 — Phase 161: Japanisch als Standard für unbekannte Audio-Sprache

Der Auftraggeber hat Japanisch als Standard bei unbekannter Sprache gewünscht und auf Rückfrage ausdrücklich auf Audio begrenzt. Eine erkannte Audio-Sprache hat Vorrang. Die vorhandene Release-Anzeige verwendet bei fehlender Audio-Sprachangabe Japanisch; Untertitel erhalten diesen Standard nicht. Die normalisierte Jellyfin-Quellinformation bleibt bei unbekannter Sprache null, damit der Anzeigestandard nicht als vom Provider gelieferte Tatsache gespeichert wird. Es werden keine Spuren erfunden und keine bestehenden Datensätze pauschal geändert. Umsetzung und Tests gehören zu Plan 161-08.


## 2026-09-15 — Phase 161: ausgewählte Quelle, öffentliche Grenze und Verifikationsumfang

Jellyfin Item-ID und MediaSource-ID bleiben unterschiedliche Identitäten. Der bestehende private JSONB-Abschnitt stream_sources.metadata.jellyfin_source hält die geprüfte Sourcebindung; keine neue Tabelle oder Datenübernahme. Gespeicherte ID hat Vorrang, nur ein eindeutiger vollständiger gespeicherter Pfad darf einen ID-Wechsel auffangen. Ohne Bindung gelten eigener Itempfad oder einzige Source, sonst Konflikt. Die elf 11eyes-Sources ohne eigenständiges Item werden nicht als neue Item-IDs oder zusätzliche Importkandidaten erfunden.

Playback und öffentliche technische Fakten verwenden eine gemeinsame Auswahl der eigenen Release-Variante/Source in einem SQL-Snapshot. Default ist die kleinste eigene Varianten-ID, dann bevorzugter Jellyfin-Stream und Stream-ID innerhalb dieser Variante; ausdrückliche Varianten bleiben an ihre echte Release-Version gebunden. Source-Identität geht in vorhandene Cachekeys/Fingerprints ein und wird vor Cachelookup bzw. Worker-Ausgabe geprüft. Die frühere allgemeine Mehrdeutigkeitsnotiz zur Compatibility-Auswahl aus 159 wird für diesen jetzt ausdrücklich deterministischen Default konkretisiert, ohne eine neue Varianten-/Release-Identität einzuführen.

Die öffentliche Einzelversionsantwort lässt die interne MediaSource-ID weg; autorisierte Editor-/Create-/Updateantworten behalten den Selector. Gespeicherte Jellyfin-URLs werden am gemeinsamen DTO-Mapper nur für die Ausgabe bereinigt, statt alte Appzeilen umzuschreiben. Nach einem Editor-Save bildet die tatsächliche Serverantwort die Baseline; nur seit Submit unveränderte Formularwerte werden abgeglichen, spätere Änderungen und neuere Dateientwürfe bleiben erhalten.

Fachlich unbekannte Audio-/Subtitle-Sprachen bleiben in Provider-/DB-/API-Daten null; die bereits bestätigte Japanisch-Regel gilt nur für die Audioanzeige. Complete-empty Tracks leeren alte Werte; fehlende Tracks können nur an derselben bestätigten Source vollständige ältere Evidenz erhalten. Keine öffentliche Leseanfrage erzeugt einen Snapshot-Backfill.

Technischer Abschluss: 9/9 Plans, unabhängige 16/16-Verifikation ohne offene Implementierungslücke. Breite Gates zeigen nur dokumentierte Baselinefehler und sind nach konkreten Revisionen ausgewiesen. Nicht ausgeführte Live-Imports/Relinks/Rescans und visuelle/akustische Playbackprüfungen bleiben als human_needed erhalten. Die bestehenden Nutzerabnahmen 156/157 werden weder ersetzt noch neu geöffnet; 158/159-Anime-UAT bleibt separat. Dateigröße und Kapitelhilfe sind ein eigener Folgeauftrag. Phase 160 wurde hier nicht ausgeführt.


## 2026-09-15 — Kapitelmarke und Kapitelabschnitt bei Segmenterstellung (Quick 260915-m2h)

Nach Live-UAT ausdrücklich bestätigt: Eine Kapitelmarke bezeichnet einen Beginn. Ihre Zeit darf einzeln als Segmentstart oder -ende übernommen werden; ein ganzer Kapitelabschnitt setzt beide Zeiten bis zur nächsten strikt späteren Marke. Gleichzeitige Marken begründen keinen Nullabschnitt. Beim letzten Kapitel darf nur eine bekannte Videolaufzeit als Ende dienen. Originalmarken bleiben in Millisekunden sichtbar; das vorhandene Segmentmodell übernimmt gerundete ganze Sekunden. Nicht darstellbare Intervalle bleiben gesperrt, längere Intervalle werden nicht auf die bestehende Vier-Minuten-Grenze gekürzt. Die bewusste Auswahl bleibt sichtbar, solange sie den aktuellen Formularzeiten und derselben Quellinformation entspricht. Kein automatischer Typ, Titel, Episodenbereich oder Speichervorgang; keine neue gespeicherte Kapitel-/Segmentwahrheit.


## 2026-09-16 — Jede eigenständig importierbare Jellyfin-MediaSource genau einmal (Quick 260916-ako)

Der ausdrückliche Folgeauftrag ersetzt die Importbeschränkung aus der Phase-161-Entscheidung vom 15.09.2026: Auch MediaSources ohne eigenes Episoden-Item sind echte Importkandidaten. Ihre Source-ID wird dabei niemals als erfundene Item-ID verwendet. Kandidat, Auswahl und Importcommand tragen das Paar aus tatsächlicher Item-ID und MediaSource-ID. Der Server prüft beim Anwenden das echte Item, seine aktuelle Source-Mitgliedschaft und den Animekontext erneut; Browserpfade und technische Browsermetadaten sind keine Evidenz.

Innerhalb des bestehenden einzelnen Jellyfin-Servers bezeichnet die MediaSource-ID die physische Quelle. Mehrere Darstellungen derselben Source werden nur bei übereinstimmendem autoritativem normalisiertem Pfad und kompatiblem Episodenkontext zusammengeführt. Bevorzugter Repräsentant ist ein echtes eigenständiges Item mit derselben ID, ansonsten der deterministisch gewählte tatsächliche Besitzer. Unterschiedliche Source-IDs bleiben getrennt, auch bei gleichem Dateipfad. Fehlende oder widersprüchliche Source-IDs werden abgewiesen; es gibt keine erfundene öffentliche Pfadidentität.

Die vorhandene private Wahrheit `stream_sources.metadata.jellyfin_source` bleibt bestehen. Migration 0166 ersetzt die bisherige allgemeine Provider-/Item-Eindeutigkeit durch getrennte Schranken für ausgewählte Jellyfin-Sources und weiterhin unaufgelöste beziehungsweise andere Providerquellen. Keine neue Tabelle, parallele Registry oder Datenübernahme. Up prüft fehlerhafte/mehrdeutige Snapshots; Down verweigert inzwischen entstandene Item-Kollisionen, statt Dateien oder Releases zusammenzulegen beziehungsweise Zeilen zu löschen. Daten dürfen nicht ohne Prüfung auf die alte Eindeutigkeit zurückgerollt werden.

Eine bestätigte Aliasdarstellung verwendet denselben vorhandenen Quell-Datensatz und erhält dessen echte Item-Zuordnung. Bestehende Importe werden pro physischer Source erkannt. Unaufgelöste Verknüpfungen dürfen nur mit eindeutigem serverseitigem Dateibeleg zugeordnet werden; zweideutige Befunde blockieren, statt alle Geschwisterquellen zu verstecken. Kein pauschaler Backfill. Gleichzeitige Importe sperren Anime und Quellen in konsistenter Reihenfolge. Lesen und Bearbeiten verwenden innerhalb der Variante dieselbe etablierte Priorität: Jellyfin vor anderen Streams, anschließend Stream-ID.

Für den geprüften 11eyes-Datensatz ergeben 27 echte Items und 52 Source-Darstellungen nach Entfernung von 14 Aliasdarstellungen 38 eigenständige Quellen, einschließlich zweier OVAs. Diese Zahl ist ein Fixturebefund, keine generelle Geschäftsregel. Zwei bestätigte Quellen desselben Items dürfen gemeinsam importiert werden; identische Paare oder doppelte physische Quellen im selben Command bleiben unzulässig. Keine Änderungen am NAS oder Jellyfin-Bestand, kein Rescan als Reparatur. Technische Verifikation dieses Quick-Auftrags ersetzt keine ausstehende menschliche Phasenabnahme.

...
// Active app memberships are canonically linked through fansub_group_members.member_id.
// A public profile must not depend on the deprecated members.user_id linkage.
func TestLoadMembershipsIncludesActiveMembershipLinkedByMemberID(t *testing.T) {
	pool := openPhase129Postgres(t)
	repo := NewMemberProfileRepository(pool, "")
	...
	require.Equal(t, int64(1293003), memberships[0].FansubGroupID)
	require.True(t, memberships[0].IsCurrent)
}

...
func openEpisodeImportSourceFixture(t *testing.T) *pgxpool.Pool {
 ...
 CREATE TABLE episode_filler_types(id BIGINT PRIMARY KEY,name TEXT);INSERT INTO episode_filler_types VALUES(1,'unknown'),(2,'canon');
...
func TestEpisodeImportSourceCreateRepeatAndInitialBinding(t *testing.T) {
	...
	require.NoError(t, pool.QueryRow(ctx, `
		SELECT eft.name
		FROM episodes e
		JOIN episode_filler_types eft ON eft.id = e.filler_type_id
		WHERE e.anime_id = 1 AND e.number = 1
	`).Scan(&fillerType))
	require.Equal(t, "canon", fillerType, "regular imported episodes default to Haupthandlung")

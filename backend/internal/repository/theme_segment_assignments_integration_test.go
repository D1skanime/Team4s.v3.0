...
// mit einer nil-pointer-Panik abstuerzen). DIES IST DIE dedizierte Regressionsschutz-Pruefung
// fuer den gefaehrlichsten Fehlerfall der Phase 156 (P156-03/T-156-03): ein unvollstaendiger
// Bereich darf NIEMALS als "Soll-Menge = leer" interpretiert und dadurch faelschlich zum
// Loeschen aller Zuweisungen fuehren. Seit der Umstellung auf die Soll-Ist-Synchronisation
// (Phase 156) muss der Guard ZUSAETZLICH beweisen, dass er VOR der neuen Loesch-Seite greift --
// nicht nur vor der Insert-Seite. Jeder Testfall prueft explizit sowohl `result == nil` als auch
// `err == nil` mit dem NEUEN Rueckgabetyp (*models.ThemeSegmentAssignmentSyncResult).
func TestAssignThemeSegmentToEpisodeRangeGuardsInvalidRangeWithoutDBAccess(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err, "ein unvollstaendiger Bereich darf NIE einen Fehler ausloesen (waere ein Symptom von DB-Zugriff)")
			require.Nil(t, got, "ein unvollstaendiger Bereich darf NIE ein Sync-Ergebnis liefern -- insbesondere KEINE Loeschung ausfuehren")
		})
	}
}
...
func TestListAnimeSegmentsAssignedEpisodesHasOverridePerEpisode(t *testing.T) {
	...
	rangeResult, err := repo.AssignThemeSegmentToEpisodeRange(ctx, segmentID, animeID, fansubGroupID, "v1", 1, 12)
	require.NoError(t, err)
	require.Len(t, rangeResult.Added, 12)

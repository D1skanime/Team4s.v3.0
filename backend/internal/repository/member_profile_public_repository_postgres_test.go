...
// --- PMDA-03 / PMDA-10: GET /members/:slug/contributions (GetPublicMemberContributionsByID) ---
//
// ACHTUNG (129-07): Diese Oberflaeche ist frontend-tot und leakt intern; Plan 129-07
// entfernt sie voraussichtlich ganz (Route + Repository-Methode + Frontend-Reste). Die
// beiden folgenden Tests stehen bewusst in eigenen Funktionen mit diesem Kommentar,
// damit sie leicht zu loeschen/verschieben sind, falls der Endpunkt entfaellt.

// TestPhase129PublicMemberContributionsRequireConfirmedStatus deckt PMDA-03 ab: der
// anime_contribution-Zweig filtert aktuell nur is_public_on_member_profile=true, NICHT
// status='confirmed'. Eine oeffentliche, aber nicht bestaetigte (draft) Contribution
// erscheint deshalb faelschlich in der Timeline (ROT). Korrekt: nur confirmed.
func TestPhase129PublicMemberContributionsRequireConfirmedStatus(t *testing.T) {
	pool := openPhase129Postgres(t)
	repo := NewAnimeContributionsRepository(pool)

	mustExecPhase129(t, pool, `
		INSERT INTO role_definitions (code, label_de) VALUES ('translator', 'Übersetzer');
		INSERT INTO members (id, nickname, public_slug) VALUES (1297001, 'phase129-confirm', 'phase129-confirm');
		INSERT INTO fansub_groups (id, slug, name, status) VALUES (1297201, 'phase129-confirm-grp', 'Phase129 Confirm Group', 'active');
		INSERT INTO hist_fansub_group_members (id, fansub_group_id, member_id, status, visibility, joined_date)
		VALUES (1297301, 1297201, 1297001, 'confirmed', 'public', '2020-01-01');
		INSERT INTO anime (id, title) VALUES (1297401, 'Phase129 Confirm Anime');
		INSERT INTO anime_contributions (id, fansub_group_id, anime_id, member_id, fansub_group_member_id, status, is_public_on_member_profile, started_year)
		VALUES (1297501, 1297201, 1297401, 1297001, 1297301, 'draft', true, 2020);
		INSERT INTO anime_contribution_roles (id, anime_contribution_id, role_code) VALUES (1297601, 1297501, 'translator');
	`)

	resp, err := repo.GetPublicMemberContributionsByID(context.Background(), 1297001)
	require.NoError(t, err)

	for _, entry := range resp.RoleTimeline {
		require.NotEqualf(t, "anime_contribution", entry.Context,
			"PMDA-03: a non-confirmed (draft) contribution must be excluded from the public timeline; leaked entry status=%q role=%q", entry.Status, entry.RoleCode)
	}
}

// TestPhase129PublicMemberContributionsExcludeRawInternalRoleCodes deckt PMDA-10 ab:
// der dritte Timeline-Zweig emittiert den ROHEN internen Code fansub_group_member_roles
// .role (z.B. 'techadmin') als role_code. Interne Permission-artige Codes duerfen nie
// in die oeffentliche Projektion leaken (ROT). Korrekt: keine rohen fgmr.role-Codes.
func TestPhase129PublicMemberContributionsExcludeRawInternalRoleCodes(t *testing.T) {
	pool := openPhase129Postgres(t)
	repo := NewAnimeContributionsRepository(pool)

	mustExecPhase129(t, pool, `
		INSERT INTO role_definitions (code, label_de) VALUES ('techadmin', 'Technischer Admin');
		INSERT INTO users (id, username, email, password_hash) VALUES (1298191, 'phase129-internal-user', 'p129internal-u@example.test', 'x');
		INSERT INTO members (id, nickname, public_slug, user_id) VALUES (1298101, 'phase129-internal', 'phase129-internal', 1298191);
		INSERT INTO app_users (id, keycloak_subject, email, display_name, legacy_user_id)
		VALUES (1298181, 'phase129-internal-kc', 'p129internal@example.test', 'Phase129 Internal', 1298191);
		INSERT INTO fansub_groups (id, slug, name, status) VALUES (1298201, 'phase129-internal-grp', 'Phase129 Internal Group', 'active');
		INSERT INTO fansub_group_members (id, fansub_group_id, app_user_id, status) VALUES (1298301, 1298201, 1298181, 'active');
		INSERT INTO fansub_group_member_roles (fansub_group_member_id, role) VALUES (1298301, 'techadmin');
	`)

	resp, err := repo.GetPublicMemberContributionsByID(context.Background(), 1298101)
	require.NoError(t, err)

	for _, entry := range resp.RoleTimeline {
		require.NotEqualf(t, "techadmin", entry.RoleCode,
			"PMDA-10: raw internal fansub_group_member_roles.role code 'techadmin' must not leak into the public timeline")
	}
}

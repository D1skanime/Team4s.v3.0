...
type AnimeCoverageRow struct {
	...
	QualifiedReleaseCnt int      `json:"qualified_release_count"`
...
func (r *AnimeCoverageRepository) CoverageByFansub(ctx context.Context, fansubGroupID int64) ([]AnimeCoverageRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.AnimeID, &row.MemberCount, &row.CoveredRoleCodes, &row.HasProjectNote, &row.HasFirstRelease, &row.QualifiedReleaseCnt, &row.HasCompletedProject, &row.HasCollaboration); err != nil {

...
func TestPhase136RoleCatalogPaletteCorrectionLiveUpDownUp(t *testing.T) {
	...
	_, err := pool.Exec(context.Background(), `
        INSERT INTO role_definitions(code, label_de, contexts, assignable) VALUES ('fansub_lead', 'Fansub-Leitung', ARRAY['fansub_group'], true);
...
func assertPhase136Palette(t testing.TB, pool *pgxpool.Pool, applied bool, auth phase136AuthorizationSnapshot) {
	...
	for code, wanted := range phase136Palette {
		...
		if applied {
			...
			if code == "typesetter" {
				require.Equal(t, "Typesetting", label)
			}
		} else {
			if code == "karaoke_fx" {
				require.Equal(t, "creative", color)
			} else {
				require.Equal(t, "other", color)
			}
			if code == "typesetter" {
				require.Equal(t, "Typesetting / FX", label)
			}
		}
	}

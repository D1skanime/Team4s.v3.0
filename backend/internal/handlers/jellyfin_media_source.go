...
func resolveJellyfinMediaSource(item jellyfinEpisodeItem, stored *models.JellyfinSourceSnapshot) (resolvedJellyfinMediaSource, error) {
 ...
 if strings.TrimSpace(item.ID)=="" { return conflict("missing","item identity absent") }
 sources:=make([]jellyfinMediaSource,0,len(item.MediaSources))
 seen:=make(map[string]bool,len(item.MediaSources))
 for _,source:=range item.MediaSources {
  source.ID=strings.TrimSpace(source.ID)
  if source.ID=="" { return conflict("missing","source identity absent") }
  if seen[source.ID] { return conflict("ambiguous","duplicate source identity") }
  seen[source.ID]=true
  source.Path=normalizeJellyfinSourcePath(source.Path)
  sources=append(sources,source)
 }
 if len(sources)==0 { return conflict("missing","no media source") }
 selected:=-1
 if stored!=nil {
  for i,source:=range sources {
   if source.ID==strings.TrimSpace(stored.MediaSourceID) { selected=i;break }
  }
 }
 if selected<0 {
  wantedPath:=normalizeJellyfinSourcePath(item.Path)
  if stored!=nil { wantedPath=normalizeJellyfinSourcePath(stored.SourcePath) }
  if wantedPath!="" {
   for i,source:=range sources {
    if source.Path!=wantedPath { continue }
    if selected>=0 { return conflict("ambiguous","multiple full path matches") }
    selected=i
   }
  }
 }
 if selected<0 {
  if stored!=nil { return conflict("missing","stored source binding lost") }
  if len(sources)!=1 { return conflict("ambiguous","no unique own source") }
  selected=0
 }
 ...
 if source.Path!="" { result.FileName=path.Base(source.Path) }
 ...
 for _,stream:=range source.MediaStreams {
  ...
  if streamIndices[stream.Index] { return conflict("ambiguous","duplicate stream index") }
 ...
 for i,track:=range result.Snapshot.AudioTracks {
  if source.DefaultAudioStreamIndex!=nil && track.Index==*source.DefaultAudioStreamIndex { selectedAudio=i;break }
 }
 if selectedAudio<0 {
  for i,track:=range result.Snapshot.AudioTracks {
   if track.IsDefault { selectedAudio=i;break }
  }
 }
 if selectedAudio<0 && len(result.Snapshot.AudioTracks)>0 { selectedAudio=0 }
 if selectedAudio>=0 {
...
func setJellyfinResolvedAudioCodec(result *resolvedJellyfinMediaSource) {
 if result.Snapshot.SelectedAudioIndex==nil { return }

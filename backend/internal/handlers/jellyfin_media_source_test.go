	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.conflict != "" {
				...
				if !errors.As(err, &conflict) || conflict.Kind != tt.conflict {
					t.Fatalf("want %s conflict, got %+v / %v", tt.conflict, got, err)
				}
			} else if err != nil || got.Snapshot.MediaSourceID != tt.wantID {
				t.Fatalf("want %s, got %+v / %v", tt.wantID, got, err)
			}
		})
	}
}
...
func TestResolveJellyfinMediaSource_StreamsPresenceAndLanguages(t *testing.T) {
	for _, tt := range []struct{ raw, want string }{
		...
		t.Run("language="+tt.raw, func(t *testing.T) {
			...
			for i := range item.MediaSources[0].MediaStreams {
				item.MediaSources[0].MediaStreams[i].Language = tt.raw
			}
			got, err := resolveJellyfinMediaSource(item, nil)
			if err != nil {
				t.Fatal(err)
			}
			if len(got.Snapshot.SubtitleTracks) != 1 || derefString(got.Snapshot.SubtitleTracks[0].Language) != tt.want || derefString(got.Snapshot.AudioTracks[0].Language) != tt.want {
				t.Fatalf("language inferred/dropped: %+v", got.Snapshot)
			}
		})
	}
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil || got.Snapshot.StreamsComplete != tt.complete || len(got.Snapshot.AudioTracks) != tt.tracks {
				t.Fatalf("presence lost: %+v / %v", got, err)
			}
			if !tt.complete && !reflect.DeepEqual(got.Snapshot.AudioTracks, prior.Snapshot.AudioTracks) {
				t.Fatal("previous complete tracks erased")
			}
		})
	}
	fresh, err := resolveJellyfinMediaSource(jellyfinSourceTestItem(t, `{"Id":"item","MediaSources":[{"Id":"a"}]}`), nil)
	if err != nil || fresh.Snapshot.StreamsComplete || len(fresh.Snapshot.AudioTracks) != 0 || len(fresh.Snapshot.SubtitleTracks) != 0 {
		t.Fatalf("fabricated absent data: %+v %v", fresh, err)
	}
}
...
func TestResolveJellyfinMediaSource_AudioSelectionAndInvalidIndex(t *testing.T) {
	...
	got, err := resolveJellyfinMediaSource(item, nil)
	if err != nil || *got.Snapshot.SelectedAudioIndex != 4 || derefString(got.AudioCodec) != "opus" {
		t.Fatalf("default: %+v %v", got, err)
	}
	source.MediaStreams[2].IsDefault = true
	got, err = resolveJellyfinMediaSource(item, nil)
	if err != nil || *got.Snapshot.SelectedAudioIndex != 3 {
		t.Fatalf("lowest default: %+v %v", got, err)
	}
	for i := range source.MediaStreams {
		source.MediaStreams[i].IsDefault = false
	}
	invalid := int32(2)
	source.DefaultAudioStreamIndex = &invalid // subtitle index is not an audio default
	got, err = resolveJellyfinMediaSource(item, nil)
	if err != nil || *got.Snapshot.SelectedAudioIndex != 3 {
		t.Fatalf("lowest audio: %+v %v", got, err)
	}
	...
	if err != nil || len(got.Snapshot.AudioTracks) != 1 || len(got.Snapshot.SubtitleTracks) != 0 || *got.Snapshot.SelectedAudioIndex != 0 {
		t.Fatalf("invalid indices: %+v %v", got, err)
	}
	...
	if _, err = resolveJellyfinMediaSource(duplicate, nil); err == nil {
		t.Fatal("duplicate track indices selected by order")
	}
}
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, item := range payload.Items {
				...
				if err != nil || chosen.Snapshot.SourcePath != item.Path || chosen.JellyfinItemID != item.ID {
					t.Fatalf("own source mismatch: %+v %v", chosen, err)
				}
				if ownPaths[chosen.Snapshot.SourcePath] {
					t.Fatal("duplicated own file")
				}
				ownPaths[chosen.Snapshot.SourcePath] = true
				...
				forEachJellyfinSourceOrder(item.MediaSources, func(order []jellyfinMediaSource) {
					...
					got, err := resolveJellyfinMediaSource(item, nil)
					if err != nil || !reflect.DeepEqual(got, chosen) {
						t.Fatalf("real fixture reordered: %+v / %v", got, err)
					}
				})
				if strings.Contains(item.Path, "FlameHazeSubs") && (len(chosen.Snapshot.AudioTracks) != 1 || chosen.Snapshot.AudioTracks[0].Language != nil || len(chosen.Snapshot.SubtitleTracks) != 0 || derefString(chosen.Container) != "mp4" || derefString(chosen.AudioCodec) != "aac") {
					t.Fatalf("FHS unknown/raw metadata changed: %+v", chosen)
				}
			}
			nonItems := 0
			...
			if len(sourceIDs) != tt.sources || nonItems != tt.nonItems || len(ownPaths) != tt.items {
				t.Fatalf("inventory items=%d sources=%d non-items=%d", len(ownPaths), len(sourceIDs), nonItems)
			}
		})
	}
}
...
func TestResolveJellyfinMediaSource_IncompleteTracksStayOnStoredBinding(t *testing.T) {
	...
	if _, err = resolveJellyfinMediaSource(changed, &previous.Snapshot); err == nil {
		t.Fatal("A tracks transferred onto incomplete B")
	}
	...
	if err != nil || recovered.Snapshot.MediaSourceID != "new-a" || recovered.Snapshot.StreamsComplete || !reflect.DeepEqual(recovered.Snapshot.AudioTracks, previous.Snapshot.AudioTracks) {
...
func TestResolveJellyfinMediaSource_ChapterAttributionAndSize(t *testing.T) {
	...
	for _, alternate := range []bool{false, true} {
		...
		for i := 0; i < 2; i++ {
			...
			if projection["FileSizeBytes"] != size {
				t.Fatalf("selected %s size=%v want=%v", id, projection["FileSizeBytes"], size)
			}
			hints, ok := projection["ChapterHints"]
			if !ok || (alternate && hints != nil) || (!alternate && len(hints.([]any)) != 1) {
				t.Fatalf("source %s borrowed or lost chapters: %#v", id, hints)
			}
		}
	}
}
...
func TestResolveJellyfinMediaSource_ChapterValidation(t *testing.T) {
	for _, tt := range []struct {
		name, chapters, path, sourcePath, extraSource, runtime, size string
		want                                                         any
	}{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if !ok || !reflect.DeepEqual(hints, tt.want) {
				t.Fatalf("chapters=%#v want=%#v (present=%v)", hints, tt.want, ok)
			}
		})
	}
	for _, count := range []int{256, 257} {
		...
		got, err := resolveJellyfinMediaSource(jellyfinSourceTestItem(t, raw), nil)
		if err != nil {
			t.Fatal(err)
		}
		encoded, _ := json.Marshal(got)
		...
		json.Unmarshal(encoded, &fields)
		if count == 256 {
			if hints, ok := fields["ChapterHints"].([]any); !ok || len(hints) != 256 {
				t.Fatal("256 chapters lost")
			}
		} else if fields["ChapterHints"] != nil {
			t.Fatal("oversized chapter list accepted")
		}
	}
	for _, size := range []string{"null", "0", "-1"} {
		got, err := resolveJellyfinMediaSource(jellyfinSourceTestItem(t, `{"Id":"item","MediaSources":[{"Id":"a","Size":`+size+`}]}`), nil)
		if err != nil {
			t.Fatal(err)
		}
		encoded, _ := json.Marshal(got)
		...
		if fields["FileSizeBytes"] != nil {
			t.Fatal("nonpositive size projected")
		}
	}
}

...
func (s *ReleaseMetadataCreditService) AwardIfCompleted(ctx context.Context, versionID, actorAppUserID int64) error {
	...
	if err != nil {
		return fmt.Errorf("award release metadata completion begin: %w", err)
	}
...
FOR UPDATE OF rv, rev, fr`, versionID).Scan(&releaseVersionID, &startedOn, &completedOn, &fansubGroupID)
	...
	if err != nil {
		return fmt.Errorf("award release metadata completion load: %w", err)
	}
	if startedOn == nil || completedOn == nil {
	...
	if err != nil {
		return fmt.Errorf("award release metadata completion member: %w", err)
	}
	...
	if err = tx.Commit(ctx); err != nil {
		return fmt.Errorf("award release metadata completion commit: %w", err)
	}
	return nil
}

...
// TestAnimeCreateEnrichmentService_DuplicateAlwaysReturnsRedirect_NoBypassPossible proves D-30
// (165-18): a duplicate AniSearch ID ALWAYS redirects -- there is no longer any request field
// that can suppress this (the earlier 165-13 bypass field was removed entirely).
func TestAnimeCreateEnrichmentService_DuplicateAlwaysReturnsRedirect_NoBypassPossible(t *testing.T) {
	t.Parallel()

	service := NewAnimeCreateEnrichmentService(
		stubAniSearchFetcher{err: errors.New("FetchAnime should not be called on a duplicate")},
		stubAnimeCreateEnrichmentRepo{
			duplicate: &models.AdminAnimeSourceMatch{AnimeID: 77, Title: "Lain"},
		},
		nil,

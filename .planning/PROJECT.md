...
Phase 154 completed on 2026-09-10 (Remediation-Phase, phaseneigener P154-* Tracking-Namespace, keine v1.4-Requirement-IDs): schloss die verbliebenen P2-Befunde aus der Messreihe vom 2026-09-09 zum öffentlichen Member-Profil. Vier redundante Faktenabfragen im Profil-Aggregator wurden auf einen einzigen Ladepfad zusammengeführt (Query-Budget 20→16, live gemessen), das ungegatete Projekt-Hero-Artwork bei null Projekten folgt jetzt dem bestehenden Locked-Gating-Muster, `ResponsiveImage`s Multi-Megabyte-Original-Fallback bei Optimizer-Fehlern ist entfernt (13,8→4,77 MB Badge-Artwork-Transfer unter `AUDIT_FAIL_BADGES=1`), animierte Avatare laufen über einen einzigen budgetierten Pfad, und der Edit-Link-Konsument lädt kein zweites Vollprofil mehr (neuer schlanker `GET /members/:slug/viewer`, 56 statt 2.738 Byte). Zwei Nachmessungen (RCA-07: leere Commits 1.664/257→0; Listener-Rest: unverändert, dokumentierter Negativbefund) und der von Phase 153 übernommene Live-Checkpoint (Owner-Ansicht eines versteckten Profils, mangels `private`-Datensatz per Ad-hoc-Toggle auf Member `d1sk` geprüft, mit präziser Abgrenzung dessen was bestätigt wurde) schließen die Phase ab. 15/15 Verifikations-Must-haves, volle Suiten grün, 0 kritische Code-Review-Befunde (2 Warnungen als Folgearbeit vorgemerkt). RCA-04 (gemeldeter Chrome-Tab-Absturz) bleibt offen und unreproduziert.

Phase 155 completed on 2026-09-11 (additiv an v1.4 angehängt, phaseneigener P155-* Tracking-Namespace, KEIN Milestone-Reset — letzte Phase der v1.4-Roadmap): die öffentliche Fansub-Projektseite lädt Identität und Navigation jetzt über einen neuen, schlanken Project Resolver (`GET /fansub-slugs/:slug/projects/:animeSlug/resolve`, konstant 2 SQL-Abfragen unabhängig von der Projektanzahl der Gruppe) statt über einen vollständigen Public-Fansub-Profile-Load — verdrahtet auf allen drei Pretty-Routes (Projekt-, Mitwirkenden-, Release-Detail-Seite). Die Releases-Zahl kommt aus einer eigenständigen `COUNT(DISTINCT rev.id)`-Abfrage statt aus einem `per_page:100`-Vollabruf; die toten Themes-/Media-Fetches sowie das entkoppelte `releaseEpisodes` sind aus dem Ladepfad und dem Render-Baum entfernt. Ein Lasttest mit 30–50 Mitwirkenden bestätigt (Negativbefund, keine Änderung nötig): `GetProjectContributors` bleibt bei konstant 2 Abfragen. Ein bei dieser Messung gefundener, aus dem Scope ausgeschlossener Produktionsdaten-Bug (`contributor_roles.name`/`role_definitions.code`-Schreibweisen-Mismatch) ist dokumentiert, nicht behoben. 15/15 Verifikations-Must-haves, volle Suiten grün, 1 Code-Review-Warnung (fehlende Fehlerbehandlung bei einem neuen `getGroupDetail`-Aufruf) noch vor Phasenabschluss behoben. Vorher/Nachher-Audit unter `docs/audits/2026-09-11-fansub-project-performance/`.
...
---
*Last updated: 2026-09-11 for milestone v1.4 Capability-, Review- und Benutzerverwaltung (complete — Phase 155 was the final additive phase)*

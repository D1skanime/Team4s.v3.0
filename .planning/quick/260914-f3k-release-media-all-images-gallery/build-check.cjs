...
const prefix='/tmp/team4s-quick-f3k-build-';
const work=fs.mkdtempSync(prefix); const marker=path.join(work,'.quick-f3k-owned'); fs.writeFileSync(marker,'260914-f3k');
try {
 ...
 if (fs.realpathSync(work)===work && work.startsWith(prefix) && fs.readFileSync(marker,'utf8')==='260914-f3k') fs.rmSync(work,{recursive:true});
}

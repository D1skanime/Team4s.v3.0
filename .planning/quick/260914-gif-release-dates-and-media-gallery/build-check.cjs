...
const prefix='/tmp/team4s-quick-gif-build-';
const work=fs.mkdtempSync(prefix); const marker=path.join(work,'.quick-gif-owned'); fs.writeFileSync(marker,'260914-gif');
try {
 ...
 if (fs.realpathSync(work)===work && work.startsWith(prefix) && fs.readFileSync(marker,'utf8')==='260914-gif') fs.rmSync(work,{recursive:true});
}

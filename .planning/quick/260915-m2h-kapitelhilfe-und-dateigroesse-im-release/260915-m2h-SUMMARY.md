...
Baseline `0c688981`. User reproduced Einspiel as an end at 00:00:00, then explicitly approved distinguishing individual chapter marks from the entire chapter section. The old action selector's constant empty value also obscured successful adoption.

**Implemented:** The existing `SegmentBasicFieldsSection.tsx` now adds `Kapitelabschnitt übernehmen`. It sets both draft times, from the selected marker to the next strictly later marker; equal timestamp labels preserve their own selection identity. The last chapter uses a known video duration only. Missing ends and intervals which collapse after whole-second rounding are disabled. The existing four-minute segment guard stays visible for longer intervals; no automatic truncation or type/title/source/episode-range changes. Individual controls are now `Kapitelmarke als Start/Ende`; invalid end marks at/before Start are disabled. Chosen labels remain while the corresponding current time and hint identity agree; manual edits and replaced hints cannot display an unrelated old selection. Shared Select, time formatting, current source guards and existing container layout are reused; duplicated Start/End selector rendering is consolidated locally.

**Tests:** RED 9 failures / 97 passes (new controls absent); GREEN 106 segment tests. Final focused run: **167 passed** (106 segment, 20 workspace, 32 central refresh, 9 auth-boundary), 0 failures/skips. Scoped ESLint: 0 errors, the same 6 warnings on untouched native controls. Typecheck: only the two documented unrelated admin-anime page errors. A newly added unsupported Testing Library `exact` option was caught and removed before final verification. `git diff --check` passed. No backend/API/schema changes, so backend tests were not repeated. A full production build was not repeated: the immediately preceding isolated build already proves the unchanged admin-anime `formatEditLoadError` export blocks completion; that existing evidence is not claimed as a fresh build for this follow-up.

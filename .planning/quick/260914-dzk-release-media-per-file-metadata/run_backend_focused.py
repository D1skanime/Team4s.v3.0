...
OUT = ROOT / ".planning/quick/260914-dzk-release-media-per-file-metadata"
PG = "team4s-quick-dzk-final-db"
GO = "team4s-quick-dzk-final-go"
LABEL = "team4s.quick=260914-dzk-final"
OWNED = {}
...
try:
    ...
    OWNED[PG] = call(["docker", "run", "--detach", "--name", PG,
        ...
        "-e", "POSTGRES_DB=team4s_phase107_test_dzk", "postgres:16",
    ...
    command = ["docker", "exec", "-e", f"TEAM4S_PHASE107_TEST_DSN=postgres://postgres@{PG}:5432/team4s_phase107_test_dzk?sslmode=disable",
        GO, "go", "test", "-p", "1", "./internal/handlers", "./internal/repository",
        "-run", "RVMTitle|ReleaseVersionMediaTitle|PreviewSerializes", "-count=1", "-json"]
    started = time.time()

...
const prefix='/tmp/team4s-quick-dzk-build-';
const work=fs.mkdtempSync(prefix); const marker=path.join(work,'.quick-dzk-owned'); fs.writeFileSync(marker,'260914-dzk');
try {
 ...
 if (fs.realpathSync(work)===work && work.startsWith(prefix) && fs.readFileSync(marker,'utf8')==='260914-dzk') fs.rmSync(work,{recursive:true});
}

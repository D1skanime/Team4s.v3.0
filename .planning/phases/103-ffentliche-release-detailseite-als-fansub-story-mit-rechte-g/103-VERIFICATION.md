...
6. **Admin media:** Sheppert preview selection passed user UAT. The selected card now has a persistent success border, an `Aktuelles Vorschaubild` badge and a `Vorschau entfernen` action. Still close/reopen an owned public image, edit its description, switch the preview once and confirm the public hero update after approval.

...
decision: "Immediate UAT 3 profile-name cleanup applied: remove the editable Anzeigename field from the profile form, because account/display naming belongs to account data and is already shown read-only in the account card. Rename the editable Team4s fansub identity field to Fansub-Nick; it maps to API `fansub_name` and database column `members.nickname`. Remove the unclear hero badges for role, nick duplicate, and dirty state."

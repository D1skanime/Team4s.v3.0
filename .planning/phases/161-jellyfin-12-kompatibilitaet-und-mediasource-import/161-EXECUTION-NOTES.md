...
## Live editor finding: credential-bearing stored URL projection

Signed-in navigation through Verwaltung -> Studio -> Buddy Complex bearbeiten -> Episoden -> Version28 -> Dateien proved that the existing editor received a stored legacy stream_url containing a query credential. No source value or key is reproduced here. New transport/import URLs were already key-free; this persisted-row response was a remaining output path. Phase161 explicitly forbids Jellyfin credentials in URLs, and application-row backfills remain unauthorized. The bounded correction extends the existing central URL-stripping seam at the existing EpisodeVersion DTO projection, scoped to Jellyfin. Consumers: grouped admin list, single version, editor context, create/update read responses. Non-Jellyfin providers retain their behavior. execute07 owns this correction's helper/mapper/tests/contract; execute04 keeps08 public projection ownership. Guarded tests must prove stored rows remain unchanged, output omits credentials, selectors remain, malformed input is not echoed. This is a measured security/output gap within the current phase, not a database migration or general legacy cleanup.

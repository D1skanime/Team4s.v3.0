...
### UI-Korrekturen nach Live-Durchsicht mit dem Auftraggeber (2026-09-21)
- **D-23 (Live-Befund, verbindlich):** Heute leitet die Create-Seite bei einem AniSearch-Dublettentreffer (Laden einer AniSearch-ID, die schon zu einem Anime gehört) **sofort automatisch** auf `/admin/anime/<id>/edit` des vorhandenen Anime um; ein zuvor übernommener Jellyfin-Entwurf geht dabei still verloren (live reproduziert mit AniSearch-ID 2788 → Naruto #4). Die UI-Spec nahm fälschlich an, es erscheine nur ein Link. Die D-02-Entscheidung („Mit bestehendem Anime verbinden“ / „Trotzdem als neuen Anime anlegen“ / „Zum vorhandenen Anime“) muss genau diese automatische Umleitung ersetzen; der Entwurf (Jellyfin-Daten, Assets, Felder) bleibt erhalten, bis der Benutzer entscheidet. Research muss den tatsächlichen Umleitungs-Codepfad (Frontend oder Backend-Antwort) finden und benennen.
- **D-24 (Auftraggeber-Entscheidung):** Die Bibliotheksliste verwendet den **Kartenstil der bestehenden Jellyfin-Suche** auf der Create-Seite (Poster, Name, Jahr | Pfad, Typ | Bibliothek, Statusblock wie „Bereits importiert → Naruto (#4) → Bestehenden Anime öffnen“, Aktions-Buttons) statt der in der UI-Spec vorgeschlagenen Tabelle; vorhandene Komponente wiederverwenden/extrahieren statt neu bauen. Pro Karte: Status-Badge (Offen / Teilweise / Bereits vorhanden / Ignoriert), Aktionen „Anime anlegen“ + „Ignorieren“ bzw. „Anime öffnen“ bzw. „Nicht mehr ignorieren“.
- **D-25 (Auftraggeber-Entscheidung):** Blättern mit klassischen Seiten (Zurück/Weiter, Richtwert 50 Einträge); der Rücksprung nach Anlegen/Episoden landet auf derselben Seite mit Filter und Suche (D-11). Kein Endlos-Scrollen.
- **D-26 (Live-Befund):** Nach „Jellyfin übernehmen“ bleibt das AniSearch-Titelfeld heute leer. Die Vorbelegung mit dem Jellyfin-Namen (D-09) ist neu und gilt für den Weg aus der Bibliothek; eine Suche wird dadurch nicht automatisch ausgelöst. Heute automatisch aus Jellyfin befüllt (bleibt so): Titel, Typ (aus der Bibliothek, z. B. ova), Status, Jahr, Ordnerpfad, Beschreibung, Cover, Banner, Logo.
- **D-27 (Live-Befund, verbindlich):** `JELLYFIN_ALLOWED_LIBRARY_IDS` kommt im laufenden Backend-Container nicht an (Liste dort immer leer, `docker-compose.yml` leitet die Variable nicht durch). Der Discovery-Snapshot MUSS deshalb bei leerer Allowed-Library-Liste denselben ungefilterten globalen Weg gehen wie `searchJellyfinSeries` (ein `/Items`-Aufruf über den ganzen Server, paginiert mit `StartIndex`/`Limit` bis `TotalRecordCount`, schlanke Fields), und bei gesetzter Liste weiterhin den gefilterten Pro-Library-Weg. Discovery darf mit der heutigen Laufzeit-Konfiguration nicht leer sein. Kein `.env`-/Compose-Umbau in dieser Phase.
- **D-28 (Live-Befund, verbindlich):** Live gegen die reale Jellyfin-12-Instanz geprüft (`/Items?IncludeItemTypes=Movie` global und im Fansubs-Parent `5f65d0c8bdd71b782fc98205814a0d76`): 0 Treffer. Movie-Items existieren auf diesem Server nicht; Filme liegen als Series-Items unter `/media/Anime/Movie/Anime.Film.Sub`. `IncludeItemTypes=Series,Movie` bleibt in der Abfrage (schadet nicht, deckt andere Server-Setups ab), aber der angezeigte Typ („Serie“, „Film“, „OVA“, „Bonus“, „Spezial“) wird aus dem Bibliotheks-Pfadkontext abgeleitet (bestehende `deriveJellyfinPathContexts`/Typ-Hint wie beim heutigen Jellyfin-Übernehmen, z. B. `Anime.Film.Sub` → Film, `Anime.OVA.Sub` → OVA), nicht aus dem Jellyfin-Item-Typ. Der Übergang für Filme nach Assisted-Create (D-01/D-10) richtet sich nach diesem abgeleiteten Typ bzw. dem AniSearch-Typ `film`.
- **D-29 (Live-Befund):** Mengengerüst ist `/Items?IncludeItemTypes=Series` im Fansubs-Parent live gemessen `TotalRecordCount=2111` (nicht ~1500 wie ursprünglich angenommen). Snapshot-/Cache-Größe, Jellyfin-Paginierung beim Aufbau und Tests (Pflichttest F) auf ~2100 Series-Items auslegen.
...
### Nachtrag nach Live-UAT und unabhängigem Code-Review (2026-09-22)
- **D-30 (Auftraggeber-Entscheidung 2026-09-22, ersetzt den „Trotzdem neu anlegen"-Teil von D-02/D-20):** Eine AniSearch-ID gehört zu genau einem Anime (DB-Constraint `uq_anime_source_links_source UNIQUE(source)`); ein zweiter Anime mit derselben `anisearch:<id>` ist fachlich unmöglich und endete in der Praxis mit HTTP 500 statt einer sauberen Fehlermeldung (165-UAT.md GAP-06). Die Option **„Trotzdem als neuen Anime anlegen" entfällt ersatzlos** — sowohl in der Dubletten-Entscheidung nach AniSearch-Auswahl (D-02) als auch in der Save-Time-Prüfung unmittelbar vor dem Anlegen (D-20). Bei einem Treffer bleiben ausschließlich **„Mit bestehendem Anime verbinden"** und **„Zum vorhandenen Anime"** (Navigation ohne Schreiboperation). Wer einen anderen Anime meint, wählt in der AniSearch-Suche einen anderen Treffer — es gibt keinen Recovery-Pfad über „trotzdem neu". Speichern mit bereits vergebener AniSearch-ID liefert HTTP 409 (nicht 500) mit dem `existing_anime_id`/`existing_title`, damit das Frontend dieselbe Verbinden-Entscheidung zeigen kann. Der `force_new`/ForceNew-Bypass (Frontend-Button, Backend-Feld auf `AdminAnimeAniSearchEnrichmentRequest`, Enrich()-Gate aus 165-13) wird vollständig entfernt, nicht nur deaktiviert.
- **D-31 (Auftraggeber-Entscheidung 2026-09-22, Kandidatensuche/GAP-08):** Die AniSearch-Kandidatensuche (Trefferliste vor der Auswahl, alle Anlege-Wege — manuell, Jellyfin-Übernahme, Bibliotheks-Discovery) filtert bereits in Team4s vorhandene Anime **nicht mehr aus**. Jeder Treffer, der bereits zu einem Anime gehört (`anime_source_links` bzw. `anime.source` = `anisearch:<id>`), erscheint weiterhin in der Liste, aber mit sichtbarem Hinweis „Existiert schon als `<Titel>` (#`<id>`)". Die Auswahl eines solchen markierten Treffers führt **direkt** zur Verbinden-Entscheidung (D-30) statt zum Laden als neuer Entwurf. Der Abgleich erfolgt gebündelt (eine Batch-DB-Abfrage über alle Kandidaten-IDs der Trefferseite, kein Request pro Kandidat/kein N+1) — Wiederverwendung der bestehenden Existenzprüfungs-Bausteine aus D-03/`FindAnimeBySource`.

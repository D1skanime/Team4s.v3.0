...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 119-01-01 | 01 | 0 | D-01–D-06, D-14 | T-119-02 | Every badge code has one canonical family; unknown earned specials render once and no category duplicates occur | Vitest unit | `docker compose exec -T team4sv30-frontend npm test -- --run src/components/profile/memberBadgeLabels.test.ts src/components/profile/MemberBadgeChain.test.tsx` | ✅ extend | ⬜ pending |
| 119-01-02 | 01 | 0 | D-11, D-15–D-16 | T-119-03 | Single-card mode has no dead controls; motion/scroll work is bounded and cancelled; grid focus is restored per carousel instance | Vitest component | `docker compose exec -T team4sv30-frontend npm test -- --run src/components/ui/FocalCarousel.test.tsx src/components/fansubs/__tests__/FansubProjectsGrid.test.tsx` | ✅ extend | ⬜ pending |
| 119-02-01 | 02 | 1 | D-01–D-04, D-07–D-08 | T-119-01 | Exact metrics are returned only inside an already-visible public profile; earned/public badge filtering remains unchanged | Go repository + contract | `docker compose exec -T team4sv30-backend go test ./internal/repository -run 'Test.*(Badge|Progress|Membership|Contribution|PublicProfile)'` | ✅ extend | ⬜ pending |
| 119-02-02 | 02 | 1 | D-01–D-04, D-06–D-08 | T-119-01, T-119-02 | Go/OpenAPI/TypeScript shapes agree; below-threshold metrics do not masquerade as earned badges | Contract + Vitest | `docker compose exec -T team4sv30-frontend npm test -- --run src/types/__tests__/v12-projection-contract.test.ts src/components/profile/memberBadgeLabels.test.ts` | ✅ extend | ⬜ pending |
| 119-02-03 | 02 | 1 | Public visibility regression | T-119-01 | Anonymous public profiles remain visible; `members_only` stays hidden from anonymous/other users and visible only to the owner | Go handler + page | `docker compose exec -T team4sv30-backend go test ./internal/handlers -run 'TestGetPublicMemberProfile' && docker compose exec -T team4sv30-frontend npm test -- --run 'src/app/members/[slug]/page.test.tsx'` | ✅ | ⬜ pending |
| 119-03-01 | 03 | 2 | D-01–D-10, D-12–D-14 | T-119-02 | Highest earned stage remains `Aktuell`; only earned stage buttons can be selected; locked stages are not tab stops | Vitest component | `docker compose exec -T team4sv30-frontend npm test -- --run src/components/profile/MemberBadgeChain.test.tsx src/components/profile/memberBadgeLabels.test.ts` | ✅ extend | ⬜ pending |
| 119-03-02 | 03 | 2 | D-11, D-15–D-16 | T-119-03 | Stage auto-centering remains inside its strip; independent grids do not share disclosure/focus state | Vitest component | `docker compose exec -T team4sv30-frontend npm test -- --run src/components/profile/MemberBadgeChain.test.tsx src/components/ui/FocalCarousel.test.tsx` | ✅ extend | ⬜ pending |
| 119-03-03 | 03 | 2 | D-05, D-13 | T-119-01, T-119-02 | Unreceived special awards and the empty special section never render; earned public specials render once with stable fallback geometry | Vitest component | `docker compose exec -T team4sv30-frontend npm test -- --run src/components/profile/MemberBadgeChain.test.tsx` | ✅ extend | ⬜ pending |
| 119-04-01 | 04 | 3 | D-01–D-16 | T-119-01–T-119-03 | Cross-consumer, contract, accessibility, visibility and responsive regressions remain green | Full automated gate | `docker compose exec -T team4sv30-frontend npm test -- --run && docker compose exec -T team4sv30-backend go test ./...` | ✅ | ⬜ pending |
| 119-04-02 | 04 | 3 | D-01–D-16 | — | Static typing, lint, production compilation and whitespace remain valid | Static/build | `docker compose exec -T team4sv30-frontend npm run typecheck && docker compose exec -T team4sv30-frontend npm run lint && docker compose exec -T team4sv30-frontend npm run build && git diff --check` | ✅ | ⬜ pending |

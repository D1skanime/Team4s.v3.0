...
| Undocumented member routes | Add or explicitly document the contributions and project-member route contracts before changing their auth/status behavior; they are absent from the current shared OpenAPI source. | [VERIFIED: server routes and OpenAPI grep] |

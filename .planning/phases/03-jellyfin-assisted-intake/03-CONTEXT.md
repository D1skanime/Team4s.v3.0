...
- The user wants the Jellyfin-assisted intake UX to feel especially polished: “ein sauberes UI” and a “tadellose” experience matter for this phase.
...
- `frontend/src/app/admin/anime/hooks/useJellyfinSync.ts` and `frontend/src/app/admin/anime/hooks/internal/useJellyfinSyncImpl.ts`: existing hook seam for Jellyfin preview/sync behavior and error handling.
...
- Jellyfin integration already has search/preview/sync client seams, so Phase 3 should focus on product behavior and UI flow rather than inventing new transport contracts.

### Integration Points
- The phase should plug into the existing `/admin/anime/create` flow instead of creating a fully separate product surface.
- Jellyfin source selection should eventually hand off into the same draft-state model now used for manual create.

...
- **D-04:** Backend-Legacy-Abbau ist voll in 106 (Liste siehe §6 des Architekturentscheids, unten unter Canonical Refs zitiert): totes UUID-Schema `backend/database/migrations/001_create_media_tables.*` + `models/media_upload.go`-Upload-DTOs; `episode_version_images`-Strecke (Migration 0018, Repo-Stub, Model, Handler, Route `/releases/:id/images`); ~~`release_media`-Junction~~ **(gestrichen — siehe D-07)**; `anime.cover_image`-String + `/api/admin/upload-cover` + `cmd/migrate-covers` + `report-/remediate-cover-image.ps1`; ~~`asset_lifecycle_service.go`~~ **(gestrichen — siehe D-08)**; Dual-Upload-Legacy (`SupportsLegacyUploadSchema`/`useLegacyUploadSchema`, `media_upload_v2_compat.go`).

### release_media — Verschiebung nach Phase 108 (Option A)
- **D-07:** **`release_media` wird in Phase 106 NICHT entfernt** — Entfernung (Tabelle, Lese- und Schreibpfad, Endpoint) verschiebt sich auf **Phase 108**. PO-Entscheid 2026-07-21 („Option A") nach Plan-Checker-Befund.
  **Begründung:** `release_media` ist kein totes Legacy, sondern backt ein **lebendes öffentliches Feature**: `GET /releases/:id/assets` (`main.go:447` → `release_assets_handler.go:38` → `ListReleaseAssets`, `episode_version_repository_read_helpers.go:322`) wird vom Frontend konsumiert (`frontend/src/lib/api.ts:6405` `getReleaseAssets` → `frontend/src/app/episodes/[id]/page.tsx` rendert die „Media Assets"-Sektion). Der Schreibpfad `media_upload_storage.go:54` (`case "release"` → `CreateReleaseMedia`) befüllt dieselbe Junction. Ein Entfernen in 106 würde entweder `go build` brechen oder das FE-Verhalten still ändern — beides verletzt die Phasengrenze „kein Verhaltensumbau an Upload/Frontend" (D-03).
  **Konkret für 106:** Migration 0131 enthält **kein** `DROP TABLE release_media`; `.down.sql` muss `release_media` nicht rekonstruieren; Lesepfad (`ListReleaseAssets`, Handler, Route, Handler-Test, `runtime_authority_test.go:156-167`) und Schreibpfad (`CreateReleaseMedia` Interface `:24` / Impl `:290` / `INSERT :292` / Caller `media_upload_storage.go:54` / Mock `:84` / `DeleteMediaAsset`-Join `:373`) bleiben **unverändert**. Contract-Check führt `release_media` **nicht** als MUSS-abwesend; die SC4-Grep-Suite sucht **nicht** nach `release_media`/`CreateReleaseMedia`.
  **Konkret für 108:** Phase 108 verdrahtet die Verwendungsrelationen (u.a. `release_version_media`) und rewired dort den Lesepfad; Entfernung von `release_media` + Endpoint-Behandlung gehört dorthin. Phase 110 verifiziert die Legacy-Freiheit im E2E-Gate.

### asset_lifecycle — Verschiebung nach Phase 107 (PO-Entscheid 2026-07-22)
- **D-08:** **`asset_lifecycle` wird in Phase 106 NICHT entfernt** — Entfernung verschiebt sich auf **Phase 107**. Plan 106-06 entfällt ersatzlos.
  **Begründung:** §6 begründet die Entfernung wörtlich mit „durch hash-basierte Ablage überflüssig". Die hash-basierte Ablage (`MediaFileService`, hash-basierter `storage_key`) entsteht aber erst in **Phase 107** — in 106 existiert sie nicht, also ist die Ordnerprovisionierung dort noch nicht überflüssig. §6 beschreibt einen Zielzustand, der in 106 noch nicht eingetreten ist. Dieselbe Fehlerklasse wie bei [D-07] `release_media`, nur eine Phase versetzt.
  **Konkret für 106:** `services/asset_lifecycle_service.go`, `services/asset_lifecycle_errors.go`, `models/asset_lifecycle.go` (`ProvisioningResult`), `repository/asset_lifecycle_audit.go`, `repository/asset_lifecycle_subjects.go` und die zugehörigen Tests bleiben **unverändert**. `EnsureCanonicalLayout`, die Entity-Existenzprüfung, die Audit-Schreibung nach `admin_anime_mutation_audit` und die Fehlercodes `asset_lifecycle.*` bleiben produktiv. Das `provisioning`-Feld bleibt über **alle drei Schichten** konsistent: `models/media_upload.go:43` → `shared/contracts/openapi.yaml:9058` → `frontend/src/types/admin.ts:691`. `asset_lifecycle` ist **kein** SC4-Suchbegriff mehr; der Contract-Check führt es als erwartet-**vorhanden** (Drop-Guard).

### Aktive Infrastruktur — falsche Grundannahme korrigiert
- **D-09:** **`media_assets`, `media_files`, `release_media` und `release_version_media` sind nachweislich aktive Infrastruktur** (PO-Feststellung 2026-07-22). Die frühere Annahme — auch die des Architekturentscheids §6 —, diese Tabellen bzw. Pfade seien weitgehend ungenutzt oder veraltet, **war falsch**.
  **Verbindliche Konsequenz für alle Phasen 106–110:** Keine dieser Tabellen und keiner der darauf laufenden Lese-/Schreibpfade darf entfernt, geleert oder still umgebogen werden, ohne dass zuvor ein **fertiger, verifizierter Ersatz** existiert und die Umstellung nachweislich verhaltenserhaltend ist.
  **Prüfregel für §6-Items (gilt ab sofort):** Die Behauptungen in §6 des Architekturentscheids, etwas sei „tot", „nur Cascade-genutzt" oder „überflüssig", sind **Hypothesen, keine Fakten** — sie haben sich bereits mehrfach als falsch erwiesen (`release_media`, `anime.cover_image`, `asset_lifecycle`, `ScreenshotGallery`). Vor jedem Abbau ist am realen Code zu verifizieren, ob lebende Leser/Schreiber/Consumer existieren, und ob der Ersatz in **dieser** Phase bereits vorhanden ist.

# Requirements Archive: v1.0 Admin Anime Intake
...
- [x] **INTK-02**: Admin can review and edit all proposed anime values in a draft form before any anime record is created.
...
- [x] **JFIN-03**: Admin can still see the linked Jellyfin item identity and path later while editing a Jellyfin-linked anime.
...
- [x] **OWNR-01**: Admin can see which editable metadata fields currently come from Jellyfin and which are manually maintained.
- [x] **OWNR-02**: Admin can save manual field values without later Jellyfin re-sync overwriting non-empty manually maintained values.
- [x] **OWNR-03**: Admin can intentionally clear a field and have a later Jellyfin re-sync refill that now-empty field.
- [x] **OWNR-04**: Admin can re-sync a Jellyfin-linked anime without creating a new anime record.
- [x] **OWNR-05**: Admin can see which Jellyfin-linked values would change before a re-sync is applied.

### Asset Management

- [x] **ASST-01**: Admin can see which asset slots came from Jellyfin, including cover, logo, banner, background, and background video.
- [x] **ASST-02**: Admin can remove a Jellyfin-derived asset from an individual asset slot without deleting the anime.
- [x] **ASST-03**: Admin can replace a Jellyfin-derived asset with a manual asset for the same slot.
- [x] **ASST-04**: Admin can continue using the existing cover upload flow inside the new anime intake and edit workflow.
- [x] **ASST-05**: Admin can keep manual replacement assets from being silently replaced by later Jellyfin re-sync.

### Relation Management

- [x] **RELA-01**: Admin can assign and maintain anime relation entries through backend-backed admin UI rather than direct database work.
- [x] **RELA-02**: Admin can create a relation from one anime to another existing anime.
- [x] **RELA-03**: Admin can update an existing relation.
- [x] **RELA-04**: Admin can delete an existing relation.
- [x] **RELA-05**: Admin can choose relation labels `Hauptgeschichte`, `Nebengeschichte`, `Fortsetzung`, and `Zusammenfassung`.
- [x] **RELA-06**: Admin relation actions use the existing normalized relation tables instead of introducing a second relation store.
- [x] **RELA-07**: Admin cannot create invalid relation entries such as self-links or duplicate relations.

### Admin Reliability

- [x] **RLY-01**: Admin can see validation failures clearly in the UI while creating, editing, syncing, or changing relations.
- [x] **RLY-02**: Admin can see Jellyfin fetch or sync failures clearly in the UI with enough context for quick debugging.
...
| JFIN-03 | Phase 4 | Complete |
| JFIN-04 | Phase 3 | Complete |
| JFIN-05 | Phase 3 | Complete |
| JFIN-06 | Phase 3 | Complete |
| OWNR-01 | Phase 4 | Complete |
| OWNR-02 | Phase 4 | Complete |
| OWNR-03 | Phase 4 | Complete |
| OWNR-04 | Phase 4 | Complete |
| OWNR-05 | Phase 4 | Complete |
| ASST-01 | Phase 4 | Complete |
| ASST-02 | Phase 4 | Complete |
| ASST-03 | Phase 4 | Complete |
| ASST-04 | Phase 2 | Complete |
| ASST-05 | Phase 4 | Complete |
| RELA-01 | Phase 5 | Complete |
| RELA-02 | Phase 5 | Complete |
| RELA-03 | Phase 5 | Complete |
| RELA-04 | Phase 5 | Complete |
| RELA-05 | Phase 5 | Complete |
| RELA-06 | Phase 5 | Complete |
| RELA-07 | Phase 5 | Complete |
| RLY-01 | Phase 4.1 | Complete |
| RLY-02 | Phase 4 | Complete |
...
*Last updated: 2026-04-01 after Phase 2 human verification closeout*

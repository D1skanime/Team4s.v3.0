...
- **D-03:** **Absenz wirkt — aber rein datenseitig, ohne UI-Ballast:** Ist eine Person im Override-Satz eines Releases nicht enthalten („war hier nicht dabei"), verliert sie die Bearbeitungsrechte **nur für dieses Release** — auch wenn der Projekt-Default oder die Gruppen-Rolle sie sonst gäben. Auf allen anderen Releases bleiben ihre Rechte unberührt. **Kein** „nicht dabei"-/Diff-/„A wurde durch B ersetzt"-Marker im UI: Absenz = Person fehlt schlicht in der Rollen-Liste des Releases. Keine Claim-/Vergleichs-Darstellung.
...
- **D-07:** **Panel-Aufbau (schlank, Nutzer-Korrektur):** Der Drawer zeigt **direkt eine Rollen-Liste „Rollen für diese Episode/Release"** — **vorbefüllt mit dem geerbten Projektteam** (beim Öffnen werden zunächst einfach alle Mitwirkenden in ihren Rollen gezeigt). **KEIN** separater „Vom Projekt geerbtes Team"-Block obendrüber. Pro Zeile: Rolle (mit Icon) + zugeordnete Person mit **Avatar** (Initialen/Bild) + **explizite Zeilen-Aktionen rechts**: „Rolle ändern" (Overflow/⋯) und „Entfernen" (Kübel-/Trash-Button, danger-farbig). Plus ein „Rolle / Person hinzufügen"-Button am Listenende (Person aus der Fansub-Gruppe + Rolle geben). Eine Rolle von Person A auf Person D umhängen = erst bei A entfernen, dann D zuweisen — die Zeile ändert sich einfach, **ohne** Hinweis „A war vorher hier" (UX-Workflow, kein harter Constraint — siehe D-09).
...
- Konkreter Override-Drawer (Nutzer-Vorgabe, präzisiert): Klick auf „Mitwirkende" bei z. B. Episode 2 → rechts öffnet ein Panel mit der Überschrift „Rollen für diese Episode". Die Liste ist beim Öffnen direkt mit dem geerbten Projektteam vorbefüllt (Rolle → Person mit Avatar). Pro Zeile: entfernen / neu vergeben; Person aus der Fansub-Gruppe hinzufügen + Rolle geben; unten Speichern/Abbrechen. **Bewusst weggelassen** (Nutzer): kein „geerbtes Team"-Block oben, keine „nicht dabei"-/Diff-/Claim-Anzeige — Änderungen erscheinen einfach als geänderte Zeilen.

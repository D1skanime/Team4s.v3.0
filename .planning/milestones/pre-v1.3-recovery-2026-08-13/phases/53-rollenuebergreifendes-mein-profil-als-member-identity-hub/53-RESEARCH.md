...
- If crop primitives move to a shared media/app-neutral location, existing `MediaUpload.tsx` must be updated to import from that new location in the same change. The shared path becomes the single canonical implementation; after all consumers are moved, the old admin-local helper files should be removed rather than kept as wrappers or duplicated copies.

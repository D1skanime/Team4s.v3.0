...
**Binding implementation rule:** Keep the routes for compatibility with the current project-roster UI, but make them thin adapters over project-owned Phase-108 services. They must not directly call transaction-owning repository writes. All three generic mutation routes are project-only: POST must reject any non-NULL `release_version_id`; PATCH and DELETE must first load the target in the route context and reject it when its stored `release_version_id` is non-NULL. PATCH must additionally reject any attempt to set `release_version_id`, including NULL→non-NULL, non-NULL→NULL, and non-NULL→different-non-NULL ownership changes. A release crew may be created, changed, emptied, or removed only through the new atomic complete-set/Replace endpoint for the real release version; no generic single-row fallback remains. Status transitions use dedicated proposal/review/member commands, not generic PATCH. Confirmed project add/change/delete synchronizes inherited snapshots; non-confirmed editorial rows remain outside effective crew and earn no points. [VERIFIED: confirmed review correction; live routes; canonical ownership rules; D-01-D-04, D-14-D-15]

**Required tests:** exercise every legacy route through runtime routing. POST with non-NULL `release_version_id` is rejected without row/snapshot/ledger writes. PATCH rejects every request that carries `release_version_id` and rejects any mutation of an already release-specific target even when the patch omits that field. DELETE rejects an already release-specific target. These rejections cover NULL→non-NULL, non-NULL→NULL, non-NULL→different release, and same-release single-row mutation. Project-wide confirmed POST/PATCH/DELETE still use the project service transaction; proposed/draft/disputed/hidden project rows produce no effective crew or ledger entry. Separate Replace-endpoint tests prove it is the sole release-crew writer and atomically handles populated replacement, empty replacement, rollback, retry, and concurrency. [VERIFIED: confirmed review correction; current route surface and status enum]

    ...
    <Card className={styles.animeProjectStage} data-family={family.key} data-anime-project-stage>
      ...
      <ol className={styles.animeProjectMilestones} aria-label="Stufen für Anime-Projekte">
        {family.stages.map((stage) => {
          ...
          const content = <><span className={styles.animeProjectMarker} aria-hidden="true">{current ? '★' : stage.earned ? '●' : <Lock size={13} />}</span><span className={styles.animeProjectMilestoneName}>{label}</span><span className={styles.animeProjectThreshold}>{stage.threshold}</span>{current ? <span className={styles.currentChip}>Aktuell</span> : null}</>
          return (
    ...
    <Card className={styles.contributionAchievementStage} data-family={family.key} data-contribution-achievement-stage>
      ...
      <ol className={styles.contributionTierTrack} aria-label={`Stufen für ${family.label}`}>
        {family.stages.map((stage) => {
          ...
          const stagePresentation = getMemberBadgePresentation(stage.badge_code)
          const stageArtwork = resolveBadgeArtwork(stage.badge_code)
    ...
    <Card className={styles.membershipStage} data-family={family.key} data-membership-stage>
      ...
      <ol className={styles.membershipDurationTrack} aria-label="Dauerstufen der Mitgliedschaft" data-membership-duration-track>
        {family.stages.map((stage) => {
          ...
          const content = <><span className={styles.membershipStageArtwork}>{stageArtwork ? <ResponsiveImage src={stageArtwork} alt="" width={160} height={160} sizes={COMPACT_BADGE_SIZES} aria-hidden="true" data-achievement-art={stage.badge_code} /> : <StageIcon size={32} aria-hidden="true" />}</span><span className={styles.membershipStageName}>{stage.threshold} Jahre</span><span className={styles.membershipStageState}>{current ? 'Aktuell' : stage.earned ? 'Erreicht' : <><Lock size={12} aria-hidden="true" /> Gesperrt</>}</span></>
  ...
  return (
    <Card className={styles.pointsAchievementStage} data-family={family.key} data-points-achievement-stage>
      ...
      <ol className={styles.pointsStageTrack} aria-label="Punkte-Meilensteine">{family.stages.map((stage) => {
        ...
        const stageArtwork = resolveBadgeArtwork(stage.badge_code)
        const StageIcon = stagePresentation.Icon
        const content = <><span className={styles.pointsStageArtwork}>{stageArtwork ? <ResponsiveImage src={stageArtwork} alt="" width={160} height={160} sizes={COMPACT_BADGE_SIZES} aria-hidden="true" /> : <StageIcon size={32} aria-hidden="true" />}</span><span className={styles.pointsStageName}>Stufe: {stagePresentation.label}</span><span className={styles.pointsStageThreshold}>Ab {formatPoints(stage.threshold)} Punkten</span><span className={styles.pointsStageState}>{current ? 'Aktuell' : stage.earned ? 'Erreicht' : <><Lock size={12} aria-hidden="true" /> Gesperrt</>}</span></>

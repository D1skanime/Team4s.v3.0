...
import type { PublicGroupedEpisode, PublicGroupedEpisodesResponse, EpisodeVersion } from '@/types/episodeVersion'
...
beforeEach(() => { window.localStorage.clear(); vi.mocked(getGroupedEpisodes).mockReset() })
...
const ended = { has_more: false, next_cursor: null, row_limit: 24 }
...
describe('bounded public inventory continuation', () => {
  it('merges 125 variants over explicit pages by episode_id/variant_id, retaining counts and neutral equal-number episodes', async () => {
    ...
    for (let offset = 23; offset < 125; offset += 23) {
      ...
      vi.mocked(getGroupedEpisodes).mockResolvedValueOnce(publicPage([
  ...
  it('keeps a failed cursor retryable and deduplicates repeated pending clicks', async () => {
    ...
    vi.mocked(getGroupedEpisodes).mockReturnValueOnce(pending.promise).mockResolvedValueOnce(publicPage([]))
  ...
  it.each(['resolve', 'reject'] as const)('aborts an old anime and ignores its late %s while the new cursor is pending', async (outcome) => {
    ...
    vi.mocked(getGroupedEpisodes).mockReturnValueOnce(old.promise).mockReturnValueOnce(current.promise)
    const { rerender, unmount } = render(<FansubVersionBrowser animeID={22} fansubs={[]} episodes={[]} pagination={continued('old')} />)
    await act(async () => {})
    fireEvent.click(screen.getByRole('button', { name: 'Weitere Episoden und Versionen laden' }))
    const oldSignal = vi.mocked(getGroupedEpisodes).mock.calls[0][1]?.signal
    ...
    unmount()
    expect(vi.mocked(getGroupedEpisodes).mock.calls[1][1]?.signal?.aborted).toBe(true)

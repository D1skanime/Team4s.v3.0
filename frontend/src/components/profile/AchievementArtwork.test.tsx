...
describe('AchievementArtwork', () => {
  it('renders direct artwork through the bounded hero contract', () => {
    ...
    expect(image.getAttribute('sizes')).toBe('(min-width: 658px) 240px, (min-width: 562px) 216px, 192px')
    expect(image.closest('[data-achievement-slot]')?.getAttribute('data-achievement-size')).toBe('hero')
  })
  ...
  it('uses identical outer geometry for layered artwork and keeps decorative layers silent', () => {
    ...
    expect(screen.getByRole('img', { name: 'Mehrteilige Auszeichnung' }).getAttribute('data-achievement-art')).toBe('layered_badge')
  ...
  it('renders historical artwork with the shared intrinsic image dimensions', () => {
    const { container } = render(
      <AchievementArtwork
        descriptor={{ kind: 'direct', src: '/member-achievement-badges/special-historical_leader-v1.png' }}

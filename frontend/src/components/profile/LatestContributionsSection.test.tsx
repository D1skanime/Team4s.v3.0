...
describe('LatestContributionsSection', () => {
  ...
  it('keeps type badge and contribution title separate for note items', async () => {
    ...
    expect(screen.queryByText('Episode 01 - v2')).toBeNull()
  ...
  it('uses a full-width 16:9 object-cover media preview without internal metadata', async () => {
    ...
    expect(screen.queryByText('Episode 03 - v1')).toBeNull()
    ...
    expect(preview.textContent).not.toContain('Typesetting-/Karaoke-Beispiel')
...
it('Quick 260812-lql reserves compact container-responsive media rows', async () => {
  ...
  expect(within(row).getAllByText('Visual Stories')).toHaveLength(1)
  expect(within(row).queryByText('Episode 03 - v1')).toBeNull()

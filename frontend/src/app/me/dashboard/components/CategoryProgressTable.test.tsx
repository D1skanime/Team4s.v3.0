...
function makeData(overrides: Partial<OwnDashboardData> = {}): OwnDashboardData {
  return {
    ...
    role_volume: [],
    category_progress: [
      { family: 'contribution_archivist', current_tier: '', current_count: 0, next_threshold: null },
...
describe('CategoryProgressTable (Phase 116, D-04)', () => {
  ...
  it('rendert eine Rollen-Volumen-Zeile je role_volume-Eintrag mit Rollen-Praefix', () => {
    render(
      <CategoryProgressTable
        data={makeData({ role_volume: [{ role_code: 'translator', count: 20 }] })}
      />,
    )
  ...
  it('rendert Bildarchivpflege-Zeile verbatim aus category_progress mit Familie+Tier-Badge', () => {
    render(
      <CategoryProgressTable
        data={makeData({
          category_progress: [
            { family: 'contribution_archivist', current_tier: 'silver', current_count: 60, next_threshold: 150 },
            { family: 'contribution_chronicle', current_tier: '', current_count: 0, next_threshold: null },
            { family: 'contribution_projects', current_tier: '', current_count: 0, next_threshold: null },
          ],
        })}

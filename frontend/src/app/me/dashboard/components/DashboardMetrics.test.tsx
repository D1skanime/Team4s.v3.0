...
function makeDashboardData(overrides: Partial<OwnDashboardData> = {}): OwnDashboardData {
  return {
...
describe('DashboardMetrics (Phase 116, D-03)', () => {
  it('rendert genau 5 HeroMetrics-Kacheln in fester Reihenfolge', () => {
    const { container } = render(<DashboardMetrics data={makeDashboardData()} />)
  ...
  it('formatiert total_points > 999 mit toLocaleString("de-DE") und zeigt die anderen vier als Ganzzahlen', () => {
    render(<DashboardMetrics data={makeDashboardData({ total_points: 1234 })} />)

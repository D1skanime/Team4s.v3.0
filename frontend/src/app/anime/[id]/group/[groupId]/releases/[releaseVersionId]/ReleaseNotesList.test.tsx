...
describe('ReleaseNotesList', () => {
  it('shows one initial note per source group before filling to three', () => {
    const notes = [
      {id:1,fansub_group_id:1,member_id:1,member_name:'A',member_avatar_url:null,role_label:'Timing',role_code:'timer',body_html:'<p>A</p>',created_at:'2026-01-01'},
      {id:2,fansub_group_id:1,member_id:2,member_name:'B',member_avatar_url:null,role_label:'Timing',role_code:'timer',body_html:'<p>B</p>',created_at:'2026-01-02'},
      {id:3,fansub_group_id:2,member_id:3,member_name:'C',member_avatar_url:null,role_label:'Karaoke',role_code:'karaoke_fx',body_html:'<p>C</p>',created_at:'2026-01-03'},
      {id:4,fansub_group_id:null,member_id:4,member_name:'D',member_avatar_url:null,role_label:'Editing',role_code:'editor',body_html:'<p>D</p>',created_at:'2026-01-04'},
    ]
  ...
  it('groups whole role blocks in the responsive role grid', () => {
    render(<ReleaseNotesList animeID={1} groupID={2} releaseVersionID={3} totalCount={2} initialNotes={[{id:1,member_id:1,member_name:'Anna',member_avatar_url:null,role_label:'Übersetzung',role_code:'translator',body_html:'<p>Text A</p>',created_at:'2026-01-02'},{id:2,member_id:2,member_name:'Mika',member_avatar_url:null,role_label:'Karaoke',role_code:'karaoke_fx',body_html:'<p>Text B</p>',created_at:'2026-01-03'}]} />)
  ...
  it('keeps non-empty source groups bucketed by release role', () => {
    render(<ReleaseNotesList
      ...
      initialNotes={[
        { id: 11, fansub_group_id: 2, member_id: 1, member_name: 'Anna', member_avatar_url: null, role_label:'Übersetzung',role_code:'translator', body_html: '<p>Text A</p>', created_at: '2026-01-02' },
        { id: 12, fansub_group_id: 3, member_id: 2, member_name: 'Mika', member_avatar_url: null, role_label:'Karaoke',role_code:'karaoke_fx', body_html: '<p>Text B</p>', created_at: '2026-01-03' },
      ]}
    />)
  ...
  it('expands only the selected stable note ID and preserves it across a cursor merge', async () => {
    ...
    vi.spyOn(api, 'getGroupReleaseNotes').mockResolvedValue({
      items: [{ id: 23, fansub_group_id: 2, member_id: 3, member_name: 'Noah', member_avatar_url: null, role_label:'Timing',role_code:'timer', body_html: '<p>Cursor-Text</p>', created_at: '2026-01-04' }],
      next_cursor: null,
      has_more: false,
    })
    render(<ReleaseNotesList
      ...
      totalCount={3}
      initialNotes={[
        { id: 21, fansub_group_id: 2, member_id: 1, member_name: 'Anna', member_avatar_url: null, role_label:'Übersetzung',role_code:'translator', body_html: `<p>${longAnnaText}</p>`, created_at: '2026-01-02' },
        { id: 22, fansub_group_id: 2, member_id: 2, member_name: 'Mika', member_avatar_url: null, role_label:'Karaoke',role_code:'karaoke_fx', body_html: `<p>${longMikaText}</p>`, created_at: '2026-01-03' },
      ]}
    />)
  ...
  it('keeps existing notes visible when cursor loading fails', async () => {
    ...
    render(<ReleaseNotesList
      ...
      releaseVersionID={3}
      totalCount={2}
      initialNotes={[{ id: 31, member_id: 1, member_name: 'Anna', member_avatar_url: null, role_label:'Übersetzung',role_code:'translator', body_html: '<p>Bestehender Text</p>', created_at: '2026-01-02' }]}
    />)
  ...
  it('hydrates the timezone-sensitive German date without a mismatch', async () => {
    ...
    const notes = [{id:3,member_id:3,member_name:'Sheppert',member_avatar_url:null,role_label:'Timing',role_code:'timer',body_html:'<p>Grenzfall</p>',created_at:timestamp}]

...
interface UseEpisodeManagerBulkMutationsParams {
  ...
  setBulkProgress: Dispatch<SetStateAction<{ done: number; total: number } | null>>
  ...
  setEditFormClearFlags: Dispatch<SetStateAction<EpisodeManagerState['editFormClearFlags']>>
}
...
export function useEpisodeManagerBulkMutations({
  ...
  const applyBulkStatus = useCallback(
    ...
    [authToken, hasAuthToken, onError, onRefresh, onSuccess, options, selectedIDs, setBulkProgress, setIsApplyingBulk],
  )
    ...
    async (animeID: number) => {
      ...
      try {
        ...
        if (failed.length > 0) {
          ...
          onSuccess(`Bulk-Entfernen OK: ${removed} Episoden entfernt, geloeschte Versions-Zuordnungen: ${removedVersionLinks}.`)
        }
      } finally {
        setIsApplyingBulk(false)
        setBulkProgress(null)

...
describe('useAdminAnimeThemes', () => {
  it('loads themes and themeTypes on mount in parallel', async () => {
    await loadAdminAnimeThemesData(7)

    expect(getAdminAnimeThemes).toHaveBeenCalledWith(7)
    expect(getAdminThemeTypes).toHaveBeenCalledWith()
  })

  it('createTheme calls API with correct animeID and payload', async () => {
    const payload = { theme_type_id: 1, title: 'Test' }
    await createAdminAnimeTheme(7, payload)

    expect(createAdminAnimeTheme).toHaveBeenCalledWith(7, { theme_type_id: 1, title: 'Test' })
  })

  it('deleteTheme calls API with correct IDs', async () => {
    await deleteAdminAnimeTheme(7, 99)

    expect(deleteAdminAnimeTheme).toHaveBeenCalledWith(7, 99)
  })
})

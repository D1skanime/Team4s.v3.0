...
describe('FansubBasicInfoTab field capabilities', () => {
 it('enables only general fields for can_edit_group_general', () => { renderTab({can_edit_group_general:true}); expect(screen.getByLabelText(/Fansubgruppen-Name/)).not.toHaveProperty('disabled', true); expect(screen.getByLabelText('Land')).not.toHaveProperty('disabled', true); expect(screen.getByLabelText(/Status/)).toHaveProperty('disabled', true); expect(screen.getByLabelText('Gruppentyp')).toHaveProperty('disabled', true) })
 it('enables lifecycle fields on the broad path', () => { renderTab({can_edit_group:true}); expect(screen.getByLabelText(/Status/)).not.toHaveProperty('disabled', true); expect(screen.getByLabelText('Gruppentyp')).not.toHaveProperty('disabled', true) })
 it('keeps fields disabled without an exact right', () => { renderTab({}); expect(screen.getByLabelText(/Fansubgruppen-Name/)).toHaveProperty('disabled', true); expect(screen.getByLabelText('Land')).toHaveProperty('disabled', true); expect(screen.getByLabelText(/Status/)).toHaveProperty('disabled', true); expect(screen.getByLabelText('Gruppentyp')).toHaveProperty('disabled', true) })
 it('shows Community-Links instead of duplicate single URL fields', () => { renderTab({can_edit_technical_links:true}); expect(screen.getByText('Links')).toBeTruthy(); expect(screen.queryByLabelText('Website')).toBeNull(); expect(screen.queryByLabelText('Discord')).toBeNull(); expect(screen.queryByLabelText('IRC')).toBeNull() })
 it('hides link creation for update-only actors', () => { renderTab({can_update_group_links:true}); expect(screen.queryByRole('button',{name:'Link hinzufügen'})).toHaveProperty('disabled', true); expect(screen.getByText('Links')).toBeTruthy() })
